...
Responsabilidades:
  1. Leer el CSV histÃ³rico y publicar cada fila en MQTT (data_source='csv').
...
Mejoras:
  ...
  - Reintentos automÃ¡ticos de conexiÃ³n al broker MQTT.
...
def publish_csv(client: mqtt.Client) -> None:
    if not os.path.exists(CSV_PATH):
        logger.warning("CSV no encontrado en %s â€” omitiendo.", CSV_PATH)
        return
