...
class TestCsvParsing:
    ...
    def test_csv_has_required_columns(self):
        """El CSV de muestra debe tener los campos mínimos requeridos."""
        csv_path = os.path.join(
            os.path.dirname(__file__), "..", "data", "spotify_tracks.csv"
        )
        required_columns = {
    ...
    def test_csv_numeric_fields(self):
        """Los campos numéricos deben ser convertibles a float."""
        csv_path = os.path.join(
            os.path.dirname(__file__), "..", "data", "spotify_tracks.csv"
        )
...
class TestPayloadBuilding:
    ...
    def _build_payload(self, row: dict) -> dict:
        """Replica la lógica de construcción de payload de main.py."""
        return {
            "track_id": row.get("id", ""),
